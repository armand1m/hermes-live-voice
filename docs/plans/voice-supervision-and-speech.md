...
**Stage 2 (durable supervision) is complete** — commits `c90d6f9` (contracts: watch registry, delegated phase, agent ports), `7c7c7f6` (herdr adapter over local + mssh), `3b5f135` (durable external-agent monitor), `8502c47` (gateway wiring: voice tools, task linkage), `e83b63d` (progress policy announcements + check-ins), `b2e4f66` (idempotent delegation launch bridge + `hermes_delegate_work` plugin tool). Suite: 1051 passed / 1 skipped, typecheck, build, and all repo checks green.

**Deployed live on exodia 2026-09-24** and canaried against real herdr: a delegated claude agent launched through `POST /v1/delegations` (workspace → agent start → objective), performed its bounded objective, and the full loop was observed — monitor state changes (idle → done with honest wording), the idempotent retry reconciled to the same watch with no duplicate agent, and a spoken announcement ("The agent on exodia is working.") reached a live v11 session through Riva TTS. Canary findings fixed and regression-tested: the workspace-create envelope nests ids (`result.workspace.workspace_id` + `result.root_pane.pane_id`), scrollback reads on working alternate-screen agents fall back to `--source visible` (`agent_not_idle`), systemd needs absolute herdr/mssh paths, and Claude Code's folder-trust dialog blocks `agent start` in untrusted directories (pre-trust the delegation repository). Config: `HERMES_LIVE_EXTERNAL_WORK_ENABLED="true"` + `HERMES_LIVE_PROGRESS_ANNOUNCEMENTS="true"` live; backup at `config.env.pre-external-work-20260924-085527`. The watch store has exactly one writer — the gateway; offline edits require the service stopped. Stage 3 (measured ASR/TTS improvements: word boosting, endpointing experiments, optional sentence buffering) remains.
